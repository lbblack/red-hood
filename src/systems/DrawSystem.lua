...
function DrawSystem:process(e, dt)
	self.cam:draw(function(l, t, w, h)
		...
		elseif e.isPlayer then
			...
			local offa = 23
			local offb = 41
			-- e.slideAnimation:draw(e.sprite, e.pos.x + offx.left, e.pos.y, 0, 1, 1, 44, offy - 15)
			...
			elseif not e.isWallsliding and not e.grounded then
				-- jumping
				if e.vel.y < 0 then
					if e.platforming.dir == 'l' then
				...
				-- falling from apex
				elseif e.vel.y < 100 and e.vel.y > 0 then
					if e.platforming.dir == 'l' then
			...
			if e.isWallsliding then
				if e.platforming.dir == 'l' then
					e.wallslideAnimation:draw(e.sprite, e.pos.x + offx.left, e.pos.y, math.pi / 2, -1, 1, 39, 46)
				else
					e.wallslideAnimation:draw(e.sprite, e.pos.x + offx.right, e.pos.y, -math.pi / 2, 1, 1, 39, 27)
				end
			end

			-- landing animation
			if e.isLanding and e.grounded then
				if e.platforming.dir == 'l' then
